...
<script type="text/javascript">
    ...
    $("#id").val(parent.id);
    $("#jobPosition").val(parent.jobPosition);
    $("#company").val(date(parent.company));
    $("#myEntryTime").val(date(parent.entryTime));
    $("#salary").val(parent.salary);

    var myStudent = parent.student;
    var myCompany = parent.company;
        ...
        success:function (data) {
            ...
            if(data.success==true){
                ...
                for(var i=0;i<size;i++){
                    console.log(data.result[i].name+" "+myStudent.trim());
                    if(data.result[i].name.trim()==myStudent.trim()){
                        console.log("111");
                        $("#student").append("<option selected value="+data.result[i].name+">"+data.result[i].name+"</option>");
                    }else{
    ...
    $.ajax({
        ...
        success:function (data) {
            ...
            if(data.success==true){
                var size=data.result.length;
                for(var i=0;i<size;i++){
                    if(data.result[i].name.trim()==myCompany.trim()){
                        $("#company").append("<option selected value="+data.result[i].name+">"+data.result[i].name+"</option>");
                    }else{
    ...
    $(function(){
        ...
        $("#form-admin-add").validate({
            ...
            submitHandler:function(form){
                var index = layer.load(3);
                $(form).ajaxSubmit({
                    url: "${pageContext.request.contextPath}/employment/update",
                    type: "POST",
                    success: function (data) {
                        layer.close(index);
                        if (data.success == true) {
